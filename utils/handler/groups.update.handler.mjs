const groupsUpdateHandler = async (m, { up, q, d, conn, bot, cache, db, find }) => {
  let senderParticipant = up.key.participant,
    messageStubParams = up.messageStubParameters;
  if (m.isGc) {
    let { g } = find;
    switch (up.messageStubType) {
      case 21:
        {
          ...
          let text = q.fsub
            ...
            .replace("@admin", "@" + senderParticipant.split("@")[0]);
      ...
      case 27:
        {
          ...
          let text = (v) =>
            v
              .replace("@admin", "@" + senderParticipant.split("@")[0])
              .replace("@user", messageStubParams.map((v) => `@${v.split("@")[0]}`).join(", "));
          if (!up.key.participant)
            return conn.sendteks(m.chat, text(q.faddlink), d.f1("Notifikasi Update Group", ""), {
              mentions: await conn.ments(text(q.faddlink))
            });
          if (!up.key.participant && m.isOwn)
            return conn.sendteks(m.chat, text(q.fownerjoin), d.f1("Notifikasi Update Group", ""), {
      ...
      default:
        ...
        const p = (parse) =>
          parse.replace(
            "@jmlh",
            (stephe == 7776000 ? "90 Hari" : stephe == 604800 ? "7 Hari" : stephe == 86400 ? "24 Jam" : "") || ""
          );
