const handle = async (m, { conn, q, repl, db, bot }) => {
   ...
   if (!db.set[i][1].store[store.k] && !db.set[i][1].store[store.b]) return repl(`Store yang anda ingin add listnya tidak ada\nAdd store dengan perintah .cs *nama store*\nAtau cari Nama store di bot ini dengan perintah .liststore`);
   if (!db.set[i][1].store[store.b].creator.includes(m.sender)) return repl(`Maaf!!!\n\nAnda siapa?\nanda bukan owner dari store ini!!!`, m);
