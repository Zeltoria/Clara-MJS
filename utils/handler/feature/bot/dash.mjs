...
const handle = async (m, { q, conn, bot, repl, db }) => {
	...
	let teks = `DASHBOARD BOT\n\n`
				...
				+ `\nTotal Hit Bot : ${Object.entries(db.cmd).map(v => v[1].hit).reduce((a, b) => a + b)}\n`
				...
				+ `Top Fitur : \n${topFitur.slice(0, 6).map((v, i) => `*${i + 1}. ${v[0]}*\n ---> Command: ${v[1].first}\n ---> Hit: ${v[1].hit}\n ---> Hit today: ${v[1].hittoday}\n ---> Last Used: ${(Date.now() - v[1].lastused).timers()}`).join('\n')}`
	repl(teks)
}
