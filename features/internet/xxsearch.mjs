...
const handle = {
  ...
  master: async (m, { q, conn, repl }) => {
    ...
    await repl(q.wait);
    if (n == "deal") {
      let { data } = await axios(`https://malesin.xyz/xnxxdl?url=${qu}`);
      ...
      let list =
        data.result.length > 0
          ? data.result.map(({ title, info, link }) => [title, `.${m.command} ${link}|deal`, info])
          : [[`Wallpaper search by: ${q.name}`, "", ""]];
      conn.sendlist(
        m.chat,
        `XNXX SEARCH\n\nTotal ${data.result.length} bokep dari query/kata kunci : ${qu}`,
